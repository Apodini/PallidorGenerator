...
struct MetaModelConverter {
    ...
    func writeToFile(targetDirectory: Path, packageName: String, servers: [String]) throws -> [URL] {
        ...
        try packageFile.replacingOccurrences(of: "###PLACEHOLDER###", with: packageName).write(to: packageSwiftPath.url, atomically: true, encoding: .utf8)
        ...
        try code.replacingOccurrences(of: "###PLACEHOLDER###", with: !servers.isEmpty ? servers.description : "[]").write(to: networkManagerPath.url, atomically: true, encoding: .utf8)
        ...
        try testFile.replacingOccurrences(of: "###PLACEHOLDER###", with: packageName).write(to: testFilePath.url, atomically: true, encoding: .utf8)
        ...
        try manifestFile.replacingOccurrences(of: "###PLACEHOLDER###", with: packageName).write(to: testManifestPath.url, atomically: true, encoding: .utf8)
        ...
        try linuxFile.replacingOccurrences(of: "###PLACEHOLDER###", with: packageName).write(to: linuxMainPath.url, atomically: true, encoding: .utf8)
